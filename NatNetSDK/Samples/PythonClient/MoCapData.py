...
class RigidBodyMarker:
    ...
    def get_as_string(self, tab_str="  ", level=0):
        ...
        if isinstance(self.size, tuple):
            size_value = len(self.size)  # Get the length of the tuple (or list)
        else:
            size_value = self.size  # If it's already an integer, use it as is

        # Safely format the output with the correct size value
        out_str += "%sSize    : %3.1d\n" % (out_tab_str, size_value)
        return out_str
